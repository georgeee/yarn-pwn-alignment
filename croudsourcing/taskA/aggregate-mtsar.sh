...
curl -s "http://127.0.0.1:8080/stages/new?id=$stage&workerRanker=mtsar.processors.meta.ZenCrowd&taskAllocator=mtsar.processors.task.RandomAllocator&answerAggregator=mtsar.processors.answer.MajorityVoting&description=taskA_$poolId"
...
curl -s "http://127.0.0.1:8080/stages/new?id=$stage&answerAggregator=mtsar.processors.answer.MajorityVoting"
wget --timeout=0 --tries=1 -O majority.csv "http://127.0.0.1:8080/stages/$stage/answers/aggregations.csv"

curl -s "http://127.0.0.1:8080/stages/new?id=$stage&answerAggregator=mtsar.processors.meta.ZenCrowd"
wget --timeout=0 --tries=1 -O zencrowd.csv "http://127.0.0.1:8080/stages/$stage/answers/aggregations.csv"

curl -s "http://127.0.0.1:8080/stages/new?id=$stage&answerAggregator=mtsar.processors.answer.KOSAggregator"
